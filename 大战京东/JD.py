...
class myThread(threading.Thread):
    ...
    def run(self):
        ...
        begin_get(self.role, self.key, self.quan_detail, self.phone_jiekou, self.begintime, self.num, self.j, self.oo,
                  self.diff)
        print("退出线程：", self.num)
...
#                 yushe_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())).split(' ')[
#                                  0] + ' {}:00:00'.format(time_mec)
