        ...
        if not os.path.exists(path):
            ...
            with open(path, 'wb') as f:
                ...
                for data in tqdm(r.iter_content(chunk_size=chunk_size)):
                    ...
                    当流下载时，用Response.iter_content或许更方便些。requests.get(url)默认是下载在内存中的，
                    下载完成才存到硬盘上，可以用Response.iter_content(chunk_size=1024)　来边下载边存硬盘
...
def main():
    ...
    for i in range(10):
        ...
        for info in infos:
            ...
            print(title)
            try:
                downloader(video_url, path=r"F:\pycharm_pracise\爬虫进阶\b站小视频\爬取的小视频\{}.mp4".format(count))
